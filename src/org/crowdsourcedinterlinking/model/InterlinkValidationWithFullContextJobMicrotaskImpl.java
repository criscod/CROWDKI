...
public class InterlinkValidationWithFullContextJobMicrotaskImpl extends
		MappingValidationJobMicrotaskImpl {
	// en loadInfo hacer SI UNIT NO ES GOLDEN ENTONCES CARGAR LA INFO

	public InterlinkValidationWithFullContextJobMicrotaskImpl(boolean turned) {
		super(turned);
	}
		...
		try {
			...
			if (!turned) {
				cmlFile = new File(ConfigurationManager.getInstance()
						.getInterlinkValidationFullContextFile());
			} else {
				cmlFile = new File(ConfigurationManager.getInstance()
						.getInterlinkValidationFullContextTurnedFile());
			}
